	...
	if (iParam0 == 0)
	{
		...
		if (chal_is_goal_active(-663946200, 1055032020))
		{
			switch (iLocal_24)
			{
				...
				case 127:
				case 136:
					chal_add_goal_progress_int_by_score_id(func_74(iLocal_24, 0), 1);
					break;
			}
		}
		if (func_10(func_67()))
...
bool func_53(bool bParam0)
{
	...
	if (func_83(&uVar1, &iVar6, 4, 1583.83f, 2193.08f, 324.18f, 5f) > 0)
	{
		...
		while (iVar11 < 4)
		{
			if (does_entity_exist(&(uVar1[iVar11])))
			{
				if (bParam0)
				{
					iVar0++;
				...
				{
					set_entity_as_mission_entity(&(uVar1[iVar11]), true, false);
					delete_object(uVar1[iVar11]);
					iLocal_13++;
				}
			}
			iVar11++;
